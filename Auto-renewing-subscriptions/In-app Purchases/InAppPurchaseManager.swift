...
class InAppPurchaseManager: NSObject, SKProductsRequestDelegate, SKPaymentTransactionObserver, SKRequestDelegate {
	...
	public func setSkipTransactionProcessingDebug(_ value: Bool) {
		#if DEBUG
		skipTransactionProcessingDebug = value
		#endif
	}
	...
	func start(withHandler handler: InAppPurchaseTransactionHandler) {
		assert(transactionHandler == nil, "startWithHandler: may only be called once")
	...
	private func fetchProducts() {
		if SKPaymentQueue.canMakePayments() {
			let identifiers = transactionHandler.availableProductIdentifiers()
			let request = SKProductsRequest(productIdentifiers: Set(identifiers))
			request.delegate = self
			request.start()
		} else {
			transactionHandler.failedToFetchProducts()
		}
	}
	...
	func purchaseProduct(_ product: SKProduct, completion: InAppPurchaseCompletion?) {
		purchaseCompletion = { receipts, error in
	...
	private func handlePurchasingState(_ transaction: SKPaymentTransaction) {
		...
		#if DEBUG
		if skipTransactionProcessingDebug {
			SKPaymentQueue.default().finishTransaction(transaction)
			return
		}
		#endif

		transactionHandler.transactionChangedToPurchasingState(transaction: transaction)
	}
	...
	private func handlePurchasedState(_ transaction: SKPaymentTransaction) {
		...
		transactionsToFinalize.append(transaction)
		validateReceipt()
	}
	...
	private func handleFailedState(_ transaction: SKPaymentTransaction) {
		SKPaymentQueue.default().finishTransaction(transaction)
		purchaseCompletion?(nil, .userCancelledTransaction)
	}
	...
	private func handleDeferredState(_ transaction: SKPaymentTransaction) {
		transactionHandler.transactionChangedToDeferredState(transaction: transaction)
	}
	...
	private func validateReceipt() {
		...
		transactionHandler.validateReceipt(receipt: receipt) { receipts, error in
			...
			for receipt in receipts {
				let tuple = self.transactionsToFinalize.enumerated().first(where: {
					$0.element.transactionIdentifier == receipt.transactionID
				})
				if let tuple = tuple {
	...
	func requestDidFinish(_ request: SKRequest) {
		if request is SKReceiptRefreshRequest {
			if let url = Bundle.main.appStoreReceiptURL, FileManager.default.fileExists(atPath: url.path) {
				...
				purchaseCompletion?(nil, InAppPurchaseError.appBundleUnavailable)
			}
		}
	}

	func request(_ request: SKRequest, didFailWithError _: Error) {
		if request is SKReceiptRefreshRequest {
			purchaseCompletion?(nil, InAppPurchaseError.appBundleUnavailable)
		}
	}
